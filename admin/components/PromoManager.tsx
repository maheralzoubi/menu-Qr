...
export const PromoManager = () => {
  ...
  const handleToggle = async (promo: PromoCode) => {
    try {
      const res = await authFetch(`/api/promos/${promo._id}/toggle`, {
        method: 'PATCH',
  ...
  return (
    <div className="space-y-10">
      <div className="flex items-end justify-between">
        <div>
          <h2 className="text-4xl font-headline font-extrabold tracking-tight">Promo Codes</h2>
          <p className="text-on-surface-variant font-medium">Create and manage discount codes for customers.</p>
        </div>
        <button
      ...
      <AnimatePresence>
        {showForm && (
          ...
          >
            <h3 className="font-headline font-bold text-lg">Create Promo Code</h3>

            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wide text-on-surface-variant">Code</label>
                <input
                  type="text"
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wide text-on-surface-variant">Discount Type</label>
                <select
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wide text-on-surface-variant">Max Uses (optional)</label>
                <input
                  type="number"
              ...
              <div className="col-span-2 space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wide text-on-surface-variant">Expiry Date (optional)</label>
                <input
                  type="date"
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 text-on-surface-variant space-y-3">
          ...
          <p className="font-medium">No promo codes yet. Create one to get started.</p>
        </div>
      ) : (
        <div className="space-y-3">
          <AnimatePresence>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {expired && (
                        <span className="text-[10px] font-bold uppercase tracking-wider bg-error/10 text-error px-2 py-0.5 rounded-full">Expired</span>
                      )}
                      {!promo.isActive && !expired && (
                        <span className="text-[10px] font-bold uppercase tracking-wider bg-on-surface/10 text-on-surface-variant px-2 py-0.5 rounded-full">Inactive</span>
                      )}
                    </div>
                    <div className="flex items-center gap-4 mt-0.5 text-xs text-on-surface-variant font-medium">
                      <span className="text-primary font-bold">{formatDiscount(promo)} off</span>
