      ...
      <div className="flex-1 space-y-8 overflow-y-auto no-scrollbar pr-2">
        ...
        <div className="flex items-end justify-between">
          <div>
            ...
            <h2 className="text-4xl font-headline font-extrabold tracking-tight">Order Archive</h2>
            <p className="text-on-surface-variant font-medium">
              {result ? `${result.total.toLocaleString()} archived orders` : 'Loading…'}
            </p>
          </div>
          <button
        ...
        {result && (
          <div className="grid grid-cols-3 gap-4">
            {[
              { label: 'Total Orders',  value: result.total.toLocaleString() },
              { label: 'Shown',         value: orders.length.toLocaleString() },
              { label: 'Revenue (page)',value: `$${totalRevenue.toFixed(2)}` },
            ].map(s => (
              <div key={s.label} className="bg-surface-container-low rounded-2xl p-5 shadow-sm border border-outline-variant/10">
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="bg-surface-container-high border-none rounded-xl py-2.5 pl-10 pr-4 text-sm font-medium focus:ring-2 focus:ring-primary/20 outline-none w-56"
            />
            {search && (
              <button onClick={() => setSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-on-surface-variant/40 hover:text-on-surface">
          ...
          <div className="relative flex items-center gap-1">
            <Filter className="w-4 h-4 text-on-surface-variant/40 absolute left-3" />
            <select
              value={status}
              onChange={e => setStatus(e.target.value)}
              className="bg-surface-container-high border-none rounded-xl py-2.5 pl-9 pr-4 text-sm font-medium focus:ring-2 focus:ring-primary/20 outline-none"
            >
          ...
          {(search || status || dateFrom || dateTo || tableNumber) && (
            <button
              onClick={() => { setSearch(''); setStatus(''); setDateFrom(''); setDateTo(''); setTableNumber(''); }}
        ...
        {isLoading ? (
          ...
          <div className="text-center py-24 text-on-surface-variant/40 space-y-3">
            ...
            <p className="font-bold text-lg">No archived orders found</p>
          ...
          <div className="space-y-3">
            <AnimatePresence mode="popLayout">
              {orders.map((order, i) => (
                ...
                >
                  <div className="w-12 h-12 rounded-xl bg-surface-container-high flex items-center justify-center shrink-0 mr-5">
                    <Package className="w-5 h-5 text-on-surface-variant/60" />
                  </div>

                  <div className="flex-1 grid grid-cols-6 gap-3 min-w-0">
                    <div>
                      <p className="text-[9px] font-bold uppercase tracking-widest text-on-surface-variant opacity-60">Order ID</p>
                      <p className="font-mono font-bold text-sm">#{order._id.slice(-6).toUpperCase()}</p>
                    </div>
                    <div>
                      <p className="text-[9px] font-bold uppercase tracking-widest text-on-surface-variant opacity-60">Date</p>
                    ...
                    <div>
                      <p className="text-[9px] font-bold uppercase tracking-widest text-on-surface-variant opacity-60">Table</p>
                      <p className="font-bold text-sm">{order.tableNumber || '—'}</p>
                    </div>
                    <div>
                      <p className="text-[9px] font-bold uppercase tracking-widest text-on-surface-variant opacity-60">Customer</p>
                      <p className="font-bold text-sm truncate">{order.customerName || 'Guest'}</p>
                    </div>
                    <div>
                      <p className="text-[9px] font-bold uppercase tracking-widest text-on-surface-variant opacity-60">Items</p>
                      <p className="font-bold text-sm">{order.items.length} items</p>
                    </div>
                    <div>
                      <p className="text-[9px] font-bold uppercase tracking-widest text-on-surface-variant opacity-60">Total</p>
                      <p className="font-bold text-sm text-primary">${order.total.toFixed(2)}</p>
                    </div>
                  </div>

                  <div className="ml-4 flex items-center gap-3 shrink-0">
                    <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${statusStyle(order.status)}`}>
                      {order.status}
                    </span>
          ...
          {selectedOrder ? (
            ...
            >
              <div className="flex justify-between items-start mb-8">
                <div>
                  <h3 className="text-xl font-headline font-extrabold tracking-tight">Order Details</h3>
              ...
              <div className="space-y-6 overflow-y-auto no-scrollbar">
                <div className="flex items-center gap-3 bg-surface-container-lowest p-4 rounded-2xl">
                  ...
                  <div>
                    <p className="font-bold text-sm">{selectedOrder.customerName || 'Guest'}</p>
                    <p className="text-xs text-on-surface-variant">{selectedOrder.tableNumber ? `Table ${selectedOrder.tableNumber}` : selectedOrder.address || '—'}</p>
                  </div>
                  <span className={`ml-auto px-3 py-1 rounded-full text-[10px] font-bold uppercase ${statusStyle(selectedOrder.status)}`}>
                    {selectedOrder.status}
                  </span>
                </div>

                <div>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-3">Items</p>
                ...
                <div className="bg-primary/5 p-5 rounded-2xl space-y-3 text-sm">
                  ...
                  {selectedOrder.discount > 0 && (
                    <div className="flex justify-between">
                      <span className="text-on-surface-variant">Discount {selectedOrder.promoCode ? `(${selectedOrder.promoCode})` : ''}</span>
