...
export const ReservationManager = () => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setReservations(prev => prev.filter(r => resKey(r) !== id));
        setSelectedRes(null);
      }
    } catch (error) {
    ...
    return (
      <div className="h-full flex flex-col space-y-8">
        <div className="flex justify-between items-end">
          <div>
            <h2 className="text-4xl font-headline font-extrabold tracking-tight">Table Layout</h2>
            <p className="text-on-surface-variant font-medium">
              {todayConfirmed} confirmed reservation{todayConfirmed !== 1 ? 's' : ''} today
            </p>
          </div>
        ...
        <div className="flex-1 bg-surface-container-low rounded-4xl p-10 relative overflow-hidden border border-outline-variant/10">
          ...
          <div className="relative w-full h-full">
            ...
            <div className="absolute bottom-0 right-0 p-6 bg-surface-container-lowest/80 backdrop-blur-md rounded-3xl border border-outline-variant/10 flex gap-8">
              <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-amber-400" /><span className="text-xs font-bold">Reserved</span></div>
              <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-surface-container-highest" /><span className="text-xs font-bold">Available</span></div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="flex h-full gap-10">
      <div className="flex-1 space-y-10 overflow-y-auto no-scrollbar pr-4">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h2 className="text-4xl font-headline font-extrabold tracking-tight">Reservations</h2>
            <p className="text-on-surface-variant font-medium">Manage guest bookings and table assignments.</p>
          </div>
          <div className="flex gap-3">
            ...
            <button onClick={() => setView('map')} className="flex items-center gap-2 px-6 py-3 bg-surface-container-high rounded-xl font-bold text-sm hover:bg-surface-variant transition-all">
              <MapIcon className="w-4 h-4" /> Table Map
            </button>
          </div>
        </div>
        ...
        {isLoading ? (
          <div className="space-y-4 animate-pulse">
            {[1, 2, 3].map(i => <div key={i} className="h-20 bg-surface-container-low rounded-3xl" />)}
          </div>
        ) : (
          <div className="space-y-4">
              ...
              {filteredReservations.map((res, i) => (
                ...
                >
                  ...
                  <div className="flex-1 grid grid-cols-4 gap-4">
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant opacity-60">Guest</p>
                      <p className="font-bold text-sm truncate">{res.name}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant opacity-60">Date & Time</p>
                      <p className="font-bold text-sm">{res.date} at {res.time}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant opacity-60">Party Size</p>
                      <p className="font-bold text-sm">{res.guests} Guests</p>
                    </div>
                    <div>
                  ...
                  <ChevronRight className="w-5 h-5 text-on-surface-variant/30 group-hover:translate-x-1 transition-transform ml-6" />
                </motion.div>
              ))}
            </AnimatePresence>
            {filteredReservations.length === 0 && (
              <div className="text-center py-20 text-on-surface-variant/40">
                ...
                <p className="font-bold">{reservations.length === 0 ? 'No reservations yet' : 'No results found'}</p>
              </div>
            )}
          </div>
        )}
          ...
          {selectedRes ? (
            ...
            >
              <div className="flex justify-between items-start mb-8">
                <div>
                  <h3 className="text-2xl font-headline font-extrabold tracking-tight">Reservation</h3>
              ...
              <div className="flex-1 space-y-6 overflow-y-auto no-scrollbar">
                ...
                <section className="grid grid-cols-2 gap-3">
                  <div className="bg-surface-container-lowest p-4 rounded-2xl">
                    ...
                    <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant opacity-60">Date</p>
                    <p className="font-bold text-sm">{selectedRes.date}</p>
                  </div>
                  <div className="bg-surface-container-lowest p-4 rounded-2xl">
                    <Clock className="w-4 h-4 text-primary mb-2" />
                    <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant opacity-60">Time</p>
                  ...
                  <div className="bg-surface-container-lowest p-4 rounded-2xl col-span-2">
                    ...
                    <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant opacity-60">Party Size</p>
                    <p className="font-bold">{selectedRes.guests} Guests</p>
                  </div>
                </section>

                {selectedRes.createdAt && (
                  <p className="text-[10px] text-on-surface-variant/40 text-center">
                    Booked {new Date(selectedRes.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                  </p>
                )}
              </div>
              ...
              <div className="mt-6 grid grid-cols-2 gap-3">
                <button
                  onClick={() => handleUpdateStatus(resKey(selectedRes), 'Cancelled')}
                ...
                <button
                  onClick={() => handleUpdateStatus(resKey(selectedRes), 'Confirmed')}
              ...
              <button
                onClick={() => handleDelete(resKey(selectedRes))}
            ...
            <div className="h-full bg-surface-container-low/50 border-2 border-dashed border-outline-variant/30 rounded-4xl flex flex-col items-center justify-center p-12 text-center text-on-surface-variant/40">
              ...
              <p className="font-bold">No Reservation Selected</p>
              <p className="text-sm">Select a booking to manage it.</p>
            </div>
          )}
        </AnimatePresence>
      </div>
