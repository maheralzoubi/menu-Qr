...
import {
  ...
  Star, LogOut, TrendingUp, Settings as SettingsIcon
} from 'lucide-react';
...
export type DashboardTab = 'overview' | 'menu' | 'orders' | 'reservations' | 'reviews' | 'analytics' | 'settings';

interface UserProfile { email: string; role: string; name?: string; title?: string; avatar?: string }
    ...
    <div className="min-h-screen bg-surface flex text-on-surface">
      ...
      <aside className="h-screen w-64 fixed left-0 top-0 border-r border-surface-container bg-surface flex flex-col py-8 z-50">
        ...
        <div className="px-4 mt-auto pt-6 border-t border-surface-container">
          <button
            onClick={onLogout}
            className="w-full flex items-center gap-3 px-4 py-3 text-on-surface-variant opacity-70 hover:bg-surface-container rounded-xl transition-all"
          >
