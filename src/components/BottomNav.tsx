...
export const NavButton = ({ active, icon, label, onClick, badge }: { active: boolean, icon: ReactNode, label: string, onClick: () => void, badge?: number }) => (
  ...
  >
    ...
    {badge !== undefined && (
      <span className="absolute top-1 right-3 w-4 h-4 bg-primary text-white text-[8px] font-bold rounded-full flex items-center justify-center">
        {badge}
      </span>
    )}
  </button>
...
export const BottomNav = ({
  ...
  <nav className="w-full">
    <div className="bg-surface-container-lowest/80 backdrop-blur-2xl rounded-[2.5rem] p-2 shadow-[0_-8px_40px_-12px_rgba(0,0,0,0.1)] border border-outline-variant/20 flex justify-around items-center h-20">
      <NavButton active={activeScreen === 'menu'} icon={<BookOpen className="w-5 h-5" />} label="Menu" onClick={() => setScreen('menu')} />
      <NavButton active={activeScreen === 'reviews'} icon={<Star className="w-5 h-5" />} label="Reviews" onClick={() => setScreen('reviews')} />
      <NavButton active={activeScreen === 'status'} icon={<ReceiptText className="w-5 h-5" />} label="Orders" onClick={() => setScreen('status')} />
      <NavButton
