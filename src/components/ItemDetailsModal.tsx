...
export const ItemDetailsModal = ({ item, onClose, onAddToCart }: { item: MenuItem, onClose: () => void, onAddToCart: () => void }) => (
  <motion.div 
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
    exit={{ opacity: 0 }}
  ...
  >
    ...
    >
      <div className="relative h-72 w-full flex-shrink-0">
        ...
        <button 
          onClick={onClose}
          className="absolute top-6 right-6 w-10 h-10 rounded-full bg-black/20 backdrop-blur-md text-white flex items-center justify-center hover:bg-black/40 transition-colors"
        >
          <X className="w-6 h-6" />
        </button>
      </div>
      ...
      <div className="p-8 space-y-8 overflow-y-auto no-scrollbar">
        ...
        <section className="space-y-3">
          <h3 className="font-headline font-bold text-lg text-on-surface">Description</h3>
          <p className="text-on-surface-variant leading-relaxed text-sm">{item.longDescription}</p>
        </section>

        <section className="space-y-3">
          <h3 className="font-headline font-bold text-lg text-on-surface">Ingredients</h3>
        ...
        <section className="space-y-3 bg-error/5 p-6 rounded-2xl border border-error/10">
          <div className="flex items-center gap-2 text-error">
            ...
            <h3 className="font-headline font-bold text-lg">Allergens</h3>
          </div>
          <p className="text-error/80 text-sm font-medium">
            This dish contains: <span className="font-bold">{item.allergens.join(', ')}</span>
          </p>
        </section>
      </div>
      ...
      <div className="p-8 bg-surface border-t border-outline-variant/10">
        ...
        >
          <Plus className="w-6 h-6" /> Add to Selection
        </button>
      </div>
    </motion.div>
  </motion.div>
