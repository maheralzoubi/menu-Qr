    ...
    <div className="pt-24 pb-32 px-6 max-w-md mx-auto space-y-8">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { icon: <Calendar className="w-5 h-5" />, label: 'Reservations', value: reservations.length },
          { icon: <Star className="w-5 h-5" />, label: 'Reviews', value: 0 },
        ].map(s => (
          <div key={s.label} className="bg-surface-container-low rounded-2xl p-4 text-center space-y-2">
            ...
            {orders.slice(0, 5).map((order: any) => {
              ...
              return (
                <div key={key} className="bg-surface-container-low rounded-2xl p-4 flex justify-between items-center">
                  <div>
                    <p className="font-bold text-sm">Order #{String(key).slice(-4).toUpperCase()}</p>
