...
export const ReviewsScreen = ({ onWriteReview, restaurantId }: { onWriteReview: () => void; restaurantId: string }) => {
  ...
  return (
    ...
    <section className="mb-10 text-center">
      <h2 className="font-headline text-4xl font-extrabold text-on-surface mb-2">Guest Notes</h2>
      <p className="text-on-surface-variant font-medium mb-6">Our community's culinary journey</p>
      <div className="bg-surface-container rounded-3xl p-8 flex flex-col items-center justify-center relative overflow-hidden">
        <div className="flex items-baseline gap-1 mb-2">
          ...
          <span className="text-on-surface-variant font-bold text-lg">/ 5</span>
        </div>
        ...
        <p className="text-sm font-semibold uppercase tracking-widest text-on-surface-variant">Average Rating: {averageRating}</p>
        <div className="mt-4 px-4 py-1.5 bg-surface-container-highest rounded-full text-xs font-bold text-on-surface">Based on {reviews.length} reviews</div>
      </div>
    </section>
        ...
        <div key={review.id} className={`rounded-[2rem] overflow-hidden ${review.image ? 'bg-surface-container-low' : 'bg-surface-container-lowest border border-outline-variant/20 shadow-sm p-8'}`}>
          {review.image && (
            <div className="relative h-48 w-full">
              ...
              <div className="absolute top-4 left-4 bg-surface-container-lowest/90 backdrop-blur-md px-3 py-1.5 rounded-2xl flex items-center gap-1.5 shadow-sm">
