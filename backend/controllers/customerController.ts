...
export const register = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(201).json({ token, customer: { id: customer._id, name: customer.name, email: customer.email } });
  } catch (e) { next(e); }
};
...
export const login = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.json({ token, customer: { id: customer._id, name: customer.name, email: customer.email, status: customer.status } });
  } catch (e) { next(e); }
};
